local function create_compile_window()
    local buf = vim.api.nvim_create_buf(false, true)

    local _, cursor_column = unpack(vim.api.nvim_win_get_cursor(0))
    cursor_column = cursor_column + 1

    local win_id = vim.api.nvim_get_current_win()
    local visible_lines = vim.api.nvim_win_get_height(win_id)
    local height = 1
    local win = vim.api.nvim_open_win(buf, true, {
        col = 3,
        fixed = true,
        focusable = true,
        height = height,
        relative = 'win',
        row = visible_lines - height - 2,
        title = 'Compile',
        width = 64,
    })

    vim.api.nvim_set_option_value('bufhidden', 'delete', { buf = buf })
    vim.api.nvim_set_option_value('buftype', 'nofile', { buf = buf })
    vim.api.nvim_set_option_value('formatoptions', '', { buf = buf })
    vim.api.nvim_set_option_value('linebreak', false, { win = win })
    vim.api.nvim_set_option_value('number', false, { win = win })
    vim.api.nvim_set_option_value('relativenumber', false, { win = win })
    vim.api.nvim_set_option_value('swapfile', false, { buf = buf })
    vim.api.nvim_set_option_value('textwidth', 0, { buf = buf })
    vim.api.nvim_set_option_value('wrap', false, { win = win })

    vim.api.nvim_command 'startinsert'

    vim.keymap.set({ 'i', 'n' }, '<Esc>', function()
        vim.api.nvim_command 'stopinsert'
    end, {
        buffer = buf,
        nowait = true,
    })

    vim.keymap.set({ 'i', 'n' }, '<Esc><Esc>', function()
        vim.api.nvim_command 'stopinsert'
        vim.api.nvim_win_close(win, true)
    end, {
        buffer = buf,
        nowait = true,
    })

    vim.keymap.set({ 'i', 'n' }, '<CR>', function()
        local command = vim.api.nvim_get_current_line()

        vim.api.nvim_win_close(win, true)

        vim.schedule(function()
            vim.cmd('below Compile ' .. command)
            vim.api.nvim_command 'stopinsert'
        end)
    end, {
        buffer = buf,
        nowait = true,
    })
end

vim.keymap.set('n', '<leader>cm', create_compile_window, {
    noremap = true,
    silent = true,
})

vim.keymap.set('n', '<leader>r', '<CMD>Recompile<CR>', {
    noremap = true,
    silent = true,
})
