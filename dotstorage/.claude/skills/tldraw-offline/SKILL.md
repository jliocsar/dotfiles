...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/home/jliocsar/.config/tldraw/server.json`.
...
```bash
PORT=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).port' '/home/jliocsar/.config/tldraw/server.json'); TOKEN=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).token' '/home/jliocsar/.config/tldraw/server.json')
